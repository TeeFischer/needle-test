...
export function determineMimeTypeFromExtension(name) {
...
export async function tryDetermineMimetypeFromURL(url, opts) {
    ...
    if (useExtension) {
        ...
        if (debug)
            console.warn(`[Needle Engine] Try to use file extension to determine type: '${ext}'`);
        switch (ext) {
            case "GLTF":
                return "model/gltf+json";
            case "VRM":
    ...
    if (header?.ok) {
        ...
        if (debug)
            console.log("[Needle Engine] Determined file type from header: " + res);
        return res;
    }
    return "unknown";
}
