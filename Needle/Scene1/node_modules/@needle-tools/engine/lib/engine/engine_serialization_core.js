import { AnimationClip, BufferGeometry, Material, Mesh, Texture } from "three";
...
import { $BuiltInTypeFlag, TypeStore } from "./engine_typestore.js";
...
export function serializeObject(obj, context) {
    ...
    // TODO move this to a helper function and do a reverse lookup once
    // instead of doing it for each object
...
function deserializeObjectWithType(data, typeOrConstructor, context, typeContext, currentValue) {
    ...
    if (!typeIsFunction && currentValue) {
        if (currentValue instanceof Material)
            return currentValue;
        if (currentValue instanceof Mesh)
            return currentValue;
        if (currentValue instanceof BufferGeometry)
            return currentValue;
        if (currentValue instanceof AnimationClip)
            return currentValue;
...
export function assign(target, source, info, opts) {
