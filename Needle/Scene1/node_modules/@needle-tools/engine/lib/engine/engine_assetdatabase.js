import { Bone, BufferAttribute, BufferGeometry, InterleavedBufferAttribute, Material, Mesh, Object3D, Scene, Skeleton, SkinnedMesh, Texture, Uniform, WebGLRenderer } from "three";
 ...
 */
export function setResourceTrackingEnabled(enabled) {
...
export function setDisposable(obj, disposable) {
    if (!obj)
        return;
...
export function disposeObjectResources(obj) {
    if (!obj)
        return;
...
function free(obj) {
    ...
    if (debug || autoDispose() || trackUsageParam)
...
export function __internalNotifyObjectDestroyed(obj) {
    if (obj instanceof Mesh || obj instanceof SkinnedMesh) {
        // obj.material = {};
        // obj.geometry = {};
    }
}
const usersBuffer = new Set();
/**
 ...
 */
export function findResourceUsers(object, recursive, predicate = null, set) {
    if (!set) {
        set = usersBuffer;
        set.clear();
    }
    ...
    if (users) {
        for (const user of users) {
            ...
            if (set.has(user))
                continue;
            //  Allow filtering
            if (predicate?.call(null, user) === false)
                continue;
            set.add(user);
            if (recursive)
...
export function getResourceUserCount(object) {
...
if (allowUsageTracking) {
    addPatch(Material.prototype, "dispose", function () { onDispose(this); });
}
...
//     i.myNumber += 1;
// }, 1000);  
//# sourceMappingURL=engine_assetdatabase.js.map