...
var NEMeshBVH;
(function (NEMeshBVH) {
    function runMeshBVHRaycast(method, mesh, results, context, options) {
        if (!mesh.geometry) {
            return false;
        }
        ...
        if (mesh?.isSkinnedMesh) {
            ...
            if (!skinnedMesh.staticGenerator) {
                ...
                if (skinnedMeshBVHNeedsUpdate || timeSinceLastUpdate > interval) {
                    if (debugPhysics)
                        console.warn(`Physics: updating skinned mesh bvh for ${mesh.name} after ${timeSinceLastUpdate.toFixed(2)}ms`);
        ...
        if (method instanceof Raycaster) {
            ...
            if (geom.boundsTree) {
                ...
                if (options.allowSlowRaycastFallback === false) {
                    if (debugPhysics)
                        console.warn("Skipping raycast because no bounds tree is available and allowSlowRaycastFallback is false");
                    return false;
                }
            }
