...
import { assign, ImplementationInformation, SerializationContext } from "./engine_serialization_core.js";
...
const idProviderCache = new Map();
function recursiveCreateGuids(obj, idProvider, guidsMap, resolveGuids) {
    if (idProvider === null)
        return;
    if (!obj)
        return;
    ...
    if (obj && obj.userData && obj.userData.components) {
        for (const comp of obj.userData.components) {
            ...
            if (idProviderKey) {
                if (!idProviderCache.has(idProviderKey)) {
                    if (debug)
                        console.log("Creating InstanceIdProvider with key \"" + idProviderKey + "\" for component " + comp[originalComponentNameKey]);
                    idProviderCache.set(idProviderKey, new InstantiateIdProvider(idProviderKey));
                }
            }
            else if (debug)
    ...
    if (obj.children) {
        for (const child of obj.children) {
            recursiveCreateGuids(child, idProvider, guidsMap, resolveGuids);
        }
    }
}
const unknownComponentsBuffer = [];
async function onCreateBuiltinComponents(context, obj, deserialize, lateResolve, level) {
    if (!obj)
        return;
            ...
            for (const compData of components) {
                try {
                    ...
                    if (type !== undefined && type !== null) {
                        ...
                        if (context.context?.physics?.engine?.isInitialized === false && (instance.isCollider || instance.isRigidbody)) {
