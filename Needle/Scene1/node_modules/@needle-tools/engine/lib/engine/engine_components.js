...
export function removeComponent(go, componentInstance) {
    if (!go)
...
export function getOrAddComponent(go, typeName, init) {
...
export function addComponent(obj, componentInstance, init, opts) {
    if (typeof componentInstance === "function") {
        const instance = new componentInstance();
        if (init)
            instance.__internalNewInstanceCreated(init);
        let callAwake = true;
        if (opts?.callAwake != undefined)
...
export function destroyComponentInstance(componentInstance) {
...
let didWarnAboutComponentAccess = false;
 ...
 * ```
 */
export function getComponent(obj, componentType) {
 ...
 * ```
 */
export function getComponents(obj, componentType, arr, clearArray = true) {
    if (!arr)
        arr = [];
    if (clearArray)
        arr.length = 0;
 ...
 * ```
 */
export function getComponentInChildren(obj, componentType, includeInactive) {
    if (includeInactive === false && obj[activeInHierarchyFieldName] === false)
        return null;
    const res = getComponent(obj, componentType);
    if (includeInactive === false && res?.enabled === false)
        return null;
    if (res)
        return res;
 ...
 * ```
 */
export function getComponentsInChildren(obj, componentType, arr, clearArray = true) {
    if (!arr)
        arr = [];
    if (clearArray)
        arr.length = 0;
 ...
 * ```
 */
export function getComponentInParent(obj, componentType) {
    if (!obj)
        return null;
 ...
 * ```
 */
export function getComponentsInParent(obj, componentType, arr, clearArray = true) {
    if (!arr)
        arr = [];
    if (clearArray)
        arr.length = 0;
 ...
 * ```
 */
export function findObjectOfType(type, contextOrScene = undefined, includeInactive = true) {
    if (!type)
        return null;
    if (!contextOrScene) {
 ...
 * ```
 */
export function findObjectsOfType(type, array, contextOrScene = undefined) {
    if (!type)
        return array ?? [];
    if (!array)
        array = [];
...
//# sourceMappingURL=engine_components.js.map