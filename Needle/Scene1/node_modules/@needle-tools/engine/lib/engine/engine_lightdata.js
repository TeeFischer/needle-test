import { ShaderChunk, UniformsLib, Vector4 } from "three";
...
export class LightDataRegistry {
    ...
    tryGetSkybox(sourceId) {
        if (debugLightmap)
            console.log("[Get Skybox]", sourceId, this.map);
        return this.tryGet(sourceId, LightmapType.Skybox, 0);
    }
    tryGetReflection(sourceId) {
        if (debugLightmap)
            console.log("[Get Reflection]", sourceId, this.map);
    ...
    tryGet(sourceId, type, index) {
        ...
        if (!entry) {
            if (debugLightmap)
                console.warn(`[Lighting] No ${LightmapType[type]} texture entry for`, sourceId);
            return null;
        }
        const arr = entry.get(type);
        if (arr === undefined) {
            if (debugLightmap)
                console.warn(`[Lighting] No ${LightmapType[type]} texture for`, sourceId, "index", index);
            return null;
        }
