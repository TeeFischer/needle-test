...
export function registerHotReloadType(instance) {
    if (isApplyingChanges) {
        if (debug)
            console.warn("[Needle Engine] Hot reloading is in progress, not registering instance", instance);
        return;
    }
    if (debug)
        console.log("[Needle Engine] Registering hot reload instance", instance);
...
export function unregisterHotReloadType(instance) {
    if (isApplyingChanges) {
        if (debug)
            console.warn("[Needle Engine] Hot reloading is in progress, not unregistering instance", instance);
        return;
    }
    if (debug)
        console.log("[Needle Engine] Unregistering hot reload instance", instance);
    const type = instance.constructor;
    const name = type.name;
    ...
    for (const key of Object.keys(newModule)) {
        try {
            ...
            if (typesCount > 0)
                ...
                hotReloadMessage += " (No instances registered)";
