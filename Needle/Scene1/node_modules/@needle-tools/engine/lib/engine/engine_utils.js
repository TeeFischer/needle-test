...
export async function generateQRCode(args) {
    ...
    try {
        const img = await addOverlays(canvas, { showLogo: args.showLogo, logoSize: sizePercentage, logoPadding: paddingPercentage }).catch(_e => { });
        if (img) {
            target.innerHTML = "";
            target.append(img);
        }
...
async function addOverlays(canvas, args) {
    if (!canvas)
        return;
    // Internal settings
