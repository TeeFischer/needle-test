...
export class GroundProjectedEnv extends Behaviour {
    ...
    onEnable() {
        ...
        if (!this._watcher) {
            ...
            this._watcher.subscribeWrite(_ => {
                if (debug)
                    console.log("Background changed", this.context.scene.background);
                this._needsTextureUpdate = true;
            });
        }
    }
    ...
    updateProjection() {
        ...
        if (!this._projection || hasChanged) {
            if (debug)
                console.log("Create/Update Ground Projection", backgroundTexture.name);
            this._projection?.removeFromParent();
            try {
    ...
    _lastBlurriness = -1;
    updateBlurriness(texture, blurriness) {
        if (!this._projection) {
            return;
        }
        else if (!texture) {
        ...
        this._needsTextureUpdate = false;
        if (debug)
            console.log("Update Blurriness", blurriness);
