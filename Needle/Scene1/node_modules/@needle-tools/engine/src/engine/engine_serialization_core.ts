...
class SerializationHelper {
    register(type: Constructor<any>, ser: ITypeSerializer) {
        if (this.typeMap.has(type)) {
            ...
            if (debug) console.warn("Type: " + type + " is already registered", ser, existing);
        }
        if (debug)
...
export class ImplementationInformation {
    ...
    registerDefinedKeys(typeName: string, type: object) {
        ...
        if (this.cache[typeName] === undefined) {
            ...
            const serializable = type as ISerializable;
            ...
            if (serializable.$serializedTypes) {
                ...
                if (keys) this.cache[typeName].push(...Object.keys(serializable.$serializedTypes));
            }
            if (debug) console.log("registerDefinedKeys for " + typeName, this.cache[typeName], type);
        }
    }
